...
for (i, NR) in enumerate(NumList):
    ...
    PNGName = 'CPS8R_'+str(NumR)+'Element'
    ...
    elemType1 = mesh.ElemType(elemCode=CPS8R, secondOrderAccuracy=OFF)#CPS4R
    elemType2 = mesh.ElemType(elemCode=CPS6M, elemLibrary=STANDARD)#CPS3
...
phPlot.title.setValues(text='Result of different element number: CPS8R')
...
session.printToFile(fileName='CPS8R_Result', format=PNG, 
    canvasObjects=( vp, ))