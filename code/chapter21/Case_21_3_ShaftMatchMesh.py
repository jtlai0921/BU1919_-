...
theta_pth = [pi/2.0*i/(4*NumP) for i in range(0, 4*NumP+1)]
pthPoints = [((a_s-d_s)*sin(i),(b_s-d_s)*cos(i),0) for i in theta_pth]
...
sCurve.setValues(displayTypes=(SYMBOL,), legendLabel=dataName, 
    useDefault=OFF)
sCurve.symbolStyle.setValues(marker=HOLLOW_CIRCLE,size=2.0,color='Black')
phPlot = session.XYPlot(name='Result')
phPlot.title.setValues(text='Result of Fitting')
...
#session.printToFile(fileName='CPS4_Result', format=PNG, 
#    canvasObjects=( vp, ))