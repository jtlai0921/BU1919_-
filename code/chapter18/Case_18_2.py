...
area = 10.0#mm^2
dens = weight/length/area
modulus = 200000.0
pointC = (length, 0.0)
Mvec = (pointB[0]-pointC[0], pointB[1]-pointC[1])
Gravity = 9800.0
denote = range(1, 20)
Ratio = [i*0.001 for i in denote]
preTension = [rs*(Gravity*weight) for rs in Ratio]
preDisp = [pt/area/modulus*length for pt in preTension]
############################
ListSuccess = []
ropeMdb = Mdb(pathName='HangingChain.cae')
for (i, Dispi) in enumerate(preDisp):
    modelName = 'TrussModel'+str(i)
    ...
    ropeMdb.models[modelName].TrussSection(name='Section-Truss', 
        material='Material-Steel', area=area)
    e = p.edges
    ...
    BC2 = ropeMdb.models[modelName].DisplacementBC(name='BC-Move', fieldName='',
        createStepName='Step-PreTension', u1=Dispi, u2=0.0, ur3=UNSET, 
    ...
    a.regenerate()
    #Job and submit
    jobName = 'HangingChainTruss'+str(i)
