...
depth = 100.0#mm
YPList = [40.0, 70.0, 140.0, 200.0, 300.0, 400.0, 500.0]
ARList = []
...
MkList = [HOLLOW_CIRCLE, HOLLOW_SQUARE, HOLLOW_DIAMOND, HOLLOW_TRI,
    CROSS, XMARKER, POINT]
dataResult= session.XYData(data=zip(YPList, ARList), name='Springback',
    xValuesLabel='Yield Point(Mpa)', yValuesLabel='Spring back angle')
...
session.printOptions.setValues(rendition=GREYSCALE, vpDecorations=OFF, 
    reduceColors=False)
session.printToFile(fileName='SpringbackCompare', format=PNG, 
    canvasObjects=( vp, ))